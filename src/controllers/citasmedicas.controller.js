...
export const registrarCita = async (req, res) => {
  ...
  try {
    ...
    const citaMedicaD1 = await citasMedicas.find(busquedaD1);
    ...
    if (citaMedicaD1.length>0) return res.status(400).json({ message: "Ya registró una cita con este médico en la misma fecha:::!!!",});
